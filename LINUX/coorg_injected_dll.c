/*
 * COORG-EXECUTOR - Injected Library (DLL equivalent for Linux)
...
static void coorg_dll_init(void) {
    printf("🚀 COORG-EXECUTOR DLL injected successfully!\n");
    ...
    printf("🎯 COORG-EXECUTOR ready for script execution\n");
}
...
static void coorg_dll_cleanup(void) {
    if (global_lua_state) {
        lua_close(global_lua_state);
    }
    printf("🧹 COORG-EXECUTOR DLL cleaned up\n");
}
